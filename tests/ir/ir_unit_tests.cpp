...
void write_file_case_report(const std::string& case_name,
                            const std::string& source_path,
                            const ir::PipelineDebugResult& debug) {
    const std::filesystem::path out_dir = std::filesystem::path("tests") / "output" / "ir_parser";
    std::error_code ec;
...
Category make_parser_category() {
    ...
    cat.cases.push_back({"Parser+Adapter 冒烟", []() -> TestResult {
        const std::string case_file = "tests/ir/ir_unit_input_parser_smoke.c";
        const ir::PipelineDebugResult dbg = ir::analyze_source_file_with_debug(case_file);
        const ir::PipelineResult& r = dbg.pipeline;
        write_file_case_report("parser_smoke", case_file, dbg);
        record_last_ir_dump(r.code);

        if (!r.success) {
            return TestResult{false, "全流程失败: " + r.error_text};
        }

        if (!contains_quad(r.code, ":=", "1", "-", "a")) {
            return TestResult{false, "未看到 (:=, 1, -, a)，实际: " + dump_code(r.code)};
        }
        return TestResult{true, "通过 .c 文件完成语法链路和 AST 适配验证"};
    }});

    cat.cases.push_back({"Parser+Adapter 失败样例", []() -> TestResult {
        const std::string case_file = "tests/ir/ir_unit_input_parser_fail.c";
        const ir::PipelineDebugResult dbg = ir::analyze_source_file_with_debug(case_file);
        const ir::PipelineResult& r = dbg.pipeline;
        write_file_case_report("parser_fail", case_file, dbg);
        record_last_ir_dump(r.code);

        if (r.success) {
            return TestResult{false, "预期失败但成功"};
        }
        if (r.error_text.find("parser.parse(tokens) failed") == std::string::npos) {
            return TestResult{false, "失败类型不匹配: " + r.error_text};
        }
        return TestResult{true, "通过 .c 文件捕获 parser 失败"};
    }});
...
Category make_error_category() {
    ...
    cat.cases.push_back({"binop 缺少操作数", []() -> TestResult {
        auto bad_binop = make_node(ASTNodeType::BINOP);
        bad_binop->op = OpType::ADD;
        bad_binop->add_child(make_node(ASTNodeType::UNKNOWN));
    ...
    cat.cases.push_back({"unop 缺少操作数", []() -> TestResult {
        auto bad_unop = make_node(ASTNodeType::UNOP);
        bad_unop->op = OpType::NOT;
        bad_unop->add_child(make_node(ASTNodeType::UNKNOWN));
...
int run_ir_unit_tests() {
    ...
    for (std::size_t i = 0; i < categories.size(); ++i) {
        ...
        for (std::size_t j = 0; j < categories[i].cases.size(); ++j) {
            ...
            if (r.passed) {
                ++unit_passed;
                std::cout << "单测" << unit_total << "已通过：（" << categories[i].cases[j].name << "；" << r.detail << "）" << '\n';
            } else {
                std::cout << "单测" << unit_total << "未通过：（" << categories[i].cases[j].name << "；" << r.detail << "）" << '\n';
            }
            std::cout << "  四元式：" << get_last_ir_dump() << '\n';
        }
    }

    std::cout << "\n单元测试汇总：通过 " << unit_passed << " / " << unit_total << '\n';
    return (unit_passed == unit_total) ? 0 : 1;
}
